...
def run_live_ingestion_test():
    ...
    for item in items:
        print(f"  - {item.sequence_code}_{item.shot_code}_{item.plate_name}: {len(item.files)} files ({item.frame_range_str})")
    ...
    print(f"\n[5/5] Executing Full Ingestion Pipeline on {len(items)} plates...")
    for idx, item in enumerate(items, 1):
        ...
        print(f"  [+] Kitsu Sequence: '{seq_obj.get('name')}' (ID: {seq_obj.get('id')})")
        ...
        # D. NAS Directory & File Copy
        nas.create_shot_structure(dest_dir)
        nas.copy_sequence(item, dest_dir)
        print(f"  [+] Copied files to NAS: {dest_dir}")

        # E. Proxy Video & Kitsu Preview Upload
        mp4_path = proxy_gen.generate_proxy(item)
        print(f"  [+] Generated Proxy MP4: {mp4_path}")

        if tasks and mp4_path:
            prep_task = next((t for t in tasks if t.get("name") == "Prep" or t.get("task_type_name") == "Prep"), tasks[0])
            preview_res = kitsu.upload_preview_proxy(prep_task, mp4_path, comment=f"Plate Ingest Preview v001 ({item.plate_name})")
            print(f"  [+] Uploaded Preview to Task '{prep_task.get('name')}': Preview ID {preview_res.get('id')}")

...
(PR #5). Phase C is **in progress** (one PR when done): `services.work` grew the
workfile lifecycle (versions / next-path / `new_workfile`); the **workfile
manager** (#4, `tools/workfile_manager/`) — a DCC-agnostic GUI: pick a task, see
its workfile versions + published outputs, start the next version, open it in
the DCC, publish a render; and the **Nuke integration** (#3, `tools/dcc/nuke/`)
— Square menu + panel, Save/Open Version, Add Write, Publish Render, Load Plate,
driving `services.work` from inside Nuke 14+. Parts marked *(exists)* below
...
| 3 | **DCC integration** *(Nuke built)* | workfile · output · task preview | `work.*`, `media.publish` | `tools/dcc/nuke/` — menu + panel + SquareRead/Write; Maya/Houdini reuse `ops` |

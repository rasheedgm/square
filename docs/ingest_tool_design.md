...
Status: **built** (Phases 1–5, live-verified) — this is the as-built record +
change log. The tool is **tool #1** of the wider pipeline; see
`pipeline_architecture.md`. Phase A of that plan ports this tool onto
`square_core` (single Kitsu access point, `PathResolver`, live `ProjectConfig`,
ledger stays tool-local) — inline `> Superseded` notes flag what changes.
...
> **As-built layout below.** Phase A of the pipeline plan moves these:
> `ingest_*`, `preflight`, `folder_mapper`, `preview_metadata`, `token_parser`
> → `tools/ingest_tool/core/`; the copy engine → `square_core/storage/transfer`;
> `nas_manager` dest/slot → `square_core/paths/resolver`; `kitsu_recorder` +
> `kitsu_gateway` → `square_core/kitsu/api` (**not** the tool — "record a
> version" is shared); `kitsu_client.py` deleted. See `pipeline_architecture.md`
> §10.
...
> **Two later changes:** (1) `Already Ingested` is now derived from the **NAS
> slot**, not a ledger hash match — see "Fixes from real-UI testing" #2 below;
> the ledger only raises a `DUPLICATE_CONTENT` *warning* for content seen
> elsewhere. (2) Per the pipeline plan the ledger is a **temporary tool-local
> exception** (`tools/ingest_tool/core/ledger.py`), not generalized — its role
> folds into Kitsu `output_files` queries once those exist.
...
> **Superseded 2026-09-02 (pipeline rework, `pipeline_architecture.md` §13):**
> `config_snapshot` is **dropped** in the port to `square_core`. On resume the
> tool reads the live `ProjectConfig`. A JSON snapshot inside the session is
> itself editable between opens, so it never was a real reproducibility
> guarantee. The block below is the as-built (pre-port) shape.
...
```jsonc
{
  ...
  "config_snapshot": {           // DROPPED in the pipeline port — read live ProjectConfig
    "nas_root": "…",
    "filename_template": "…",
    "nas_dir_template": "…",
    "media_type_configs": { … },
...
> **Superseded by the pipeline plan.** `KitsuRecorder` + `kitsu_gateway` fold
> into `square_core/kitsu/api.py` — "record a version in Kitsu" is shared by
> publish and review, not ingest-only. Versions now land as `output_files`
> revisions (Kitsu owns the number; we `PUT` our path), not a `shot.data`
> `versions[]` blob. Kitsu is the store, not just written-to. The `data`
> nesting key `"square_ingest"` is renamed `"square"` in the port. See
> `pipeline_architecture.md` §5–§6 and `config_and_paths.md`. As-built below.
...
class KitsuRecorder:
    ...
    def record_version(self, shot, item, result, preview_meta) -> None
        # as built: comment on Ingest task + (preview upload + data.square_ingest
